...
def main():
    ...
    parser.add_argument("--min_period_sec", type=float, default=0.25)
    parser.add_argument("--max_period_sec", type=float, default=None)
    ...
    metrics_df = pd.DataFrame([{
        ...
        "mode": args.mode,
        "period_frames": repet["period_frames"],
        "period_seconds": repet["period_frames"] * args.hop_length / sr,
        **mask_metrics,
    ...
    metrics_path = outdir / f"{data['track_name'].replace('/', '_')}_repet_metrics.csv"
    metrics_df.to_csv(metrics_path, index=False)

    print(f"Track: {data['track_name']}")
    print(f"Sample rate: {sr}")
    print(f"Estimated repeating period: {repet['period_frames']} frames")
    print(f"Estimated repeating period: {repet['period_frames'] * args.hop_length / sr:.3f} sec")
    print("Mask metrics:")
    ...
    print(f"Saved REPET stems to: {stem_paths}")
